# 008 — Three LoadBalancer IPs per App Environment

[docs](../../index.md) / [architecture](../index.md) / [decisions](./) / 008 — Three LoadBalancer IPs per App Environment

**Date:** 2026-03-31
**Status:** accepted
**Audiences:** architect, platform engineer, network engineer

## Context

Each App Environment needs to serve three distinct traffic classes with different security requirements:

1. **Internal ops traffic** (monitoring dashboards, internal APIs) -- no external exposure needed.
2. **DFSP non-mTLS traffic** (MCM portal, Keycloak, Finance Portal) -- TLS termination at the gateway, public-facing.
...
Three LoadBalancer Services per App Environment:
...
| `cilium-gateway-gw-ext` | Cilium Gateway (`gw-ext`) | DFSP non-mTLS (MCM, Keycloak, Finance Portal) | TLS termination (wildcard cert on `*.ext.${domain}`) |
...
- **On-prem IP budget.** Small on-prem deployments must plan for at least 3 IPs in the LB-IPAM pool per App Environment, plus additional IPs for the Control Center.
