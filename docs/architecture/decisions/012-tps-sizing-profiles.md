...
**Date:** 2026-04-02
**Status:** accepted
**Audiences:** architect, platform engineer, adopter (deploy)
...
Performance testing (see `legacy/ml-perf`) showed that four dimensions must scale together for a given throughput target: the infrastructure provider, infrastructure topology, application horizontal scaling, and data layer tuning. Scaling any one without the others creates bottlenecks.
...
- `env/` — TPS-driven (`tps-1`, `tps-500`, `tps-2000`)
- `cc/` — Operations-scale-driven (`small`, `medium`, `large`)
- `base/` — Lightweight infra-only (`small`)
...
Detailed design: `doc/design-tps-profiles.md`
