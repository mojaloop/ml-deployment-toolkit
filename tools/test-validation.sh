...
expect_reject "registry provider removed"  '.registry.provider = "harbor"'
expect_reject "object_storage provider removed" '.object_storage.provider = "s3"'
expect_reject "observability provider removed"  '.observability.provider = "urls"'
expect_reject "tooling block removed"      '.tooling.domain = "cc1.example.com"'
expect_reject "registry.enabled as string" '.registry.enabled = "true"'
expect_reject "missing registry.enabled"   'del(.registry.enabled)'
expect_reject "missing object_storage"     'del(.object_storage)'
expect_reject "missing observability"      'del(.observability)'
expect_reject "missing cert.email"         'del(.cert.email)'
expect_reject "template as number"         '.template = 5'
expect_reject "lb_ipam without pools"      'del(.cluster.lb_ipam.pools)'
