...
While a cluster is still running, its generated passwords can be read back two ways — `make secrets ENV=<env>` from the config state, or the `cluster-secrets` Secret in `flux-system`. Neither survives losing both the state and the cluster, which is why the state backup covers `../artifacts/<env>/state/` in full.
...
1. Restore the environment repository to `../environments/<env>/` and `.env` from the secure copy, plus both state files under `../artifacts/<env>/state/`.
