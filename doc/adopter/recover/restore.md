...
1. **What happened** — a bad migration, a corrupted table, a lost cluster? A localized corruption is a point-in-time restore; a lost cluster is [disaster recovery](disaster-recovery.md).
...
3. **Whether the overwrite is acceptable** — a restore replaces current state. If current state holds *some* good data written after the target time, capture it first.
...
Vault is different — it is not operator-restored. The adopter restores a raft snapshot with Vault's own command, and it is more disruptive, because unsealing and the token that authorizes the restore are involved.
...
2. **Authenticate to Vault** with a token that can perform the restore. On a running Vault this is the root token; if Vault is lost, the root token comes from the adopter's [out-of-band backup](backup.md#vault-unseal-keys).
3. **Restore:**
   ```bash
...
- **Confirm the data matches expectations** — a restore to the wrong point is worse than none, because it looks successful. Spot-check known records against the target time.
