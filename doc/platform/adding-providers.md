...
- **The full set of role/capacity template files** — `<role>/<name>/` directories for every role and size the provider supports: `bare/small`, `hub/{dev,tps-1,tps-10}`, `tooling/{dev,small,medium}`. Each holds `template.yaml` (identity + tuning), `placement.yaml` (the shape: node pools, default topology), and `values/`, `patches/`, `talos/` overlay surfaces. Each is a complete overlay with no knobs or conditionals. Duplication across providers is accepted by design — copying another provider's role files and adjusting them is the intended workflow, not a shortcut.
