...
- [What you must not touch](#what-you-must-not-touch)

## The contract

A provider module does exactly one thing: **provision a cluster and return a kubeconfig.** Everything above that is identical across providers, so if you find yourself editing DNS, TLS, or application code to add a provider, something is in the wrong place.
...
2. **`local.provider_outputs`** — add your module's outputs under the provider name.
3. **`local.kubeconfig_paths`** — add your provider's kubeconfig path here.
...
4. **`flux_bootstrap` depends_on** — add your module so Flux waits for the cluster to exist.
...
The profile files are what `config.yaml`'s `profile:` field selects. Without at least one profile per role you intend to support, an adopter cannot size a cluster on your provider.
...
`gitops/talos/` is the existing example. If your provider is self-managed and needs the same class of resources, add a vendor directory and ensure `flux-config` names it for your provider. A managed provider that ships its own CNI and storage needs no vendor layer — and `has_vendor` in `flux-config` must reflect that, or Flux will look for a directory that does not exist. (An existing mismatch of exactly this kind is tracked in `discrepancies.md` item 3 — do not replicate it.)

## What you must not touch

If adding a provider makes you edit any of these, reconsider the design:
