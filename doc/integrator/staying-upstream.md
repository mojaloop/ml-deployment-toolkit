...
4. **Verify.** Run `make check` first — the `tools/checks/` suite validates substitution tokens (across the artifact and environment layers), the three-layer `valuesFrom` chains, values-file bindings, secret placement, node-role placement, hardcoded-literal hygiene, the provider interface, role-aware token resolution, and tool versions locally, and catches a whole class of rebase damage before anything deploys. After edits to HelmReleases, re-run `tools/generate-valuesfrom.sh` — the chain tail is generated, never hand-maintained. Then render offline: `tools/render.sh <env>` produces the merged values chains, the kustomize build of every layer, and the Talos fragment validation with no cluster, so a rebase's effect is inspectable as a diff of `../artifacts/<env>/render/`. None of this is a substitute for the real thing: there is no hosted CI, so verification proper is a real deployment to a lab environment, a walk through the [adopter verification](../adopter/deploy/deployment.md#verify-up-the-stack), and a test transaction.
