...
> **Amended 2026-08-15 (config-layering refactor, second wave).** Two paths in the previous amendment have moved again. State now lives at `../artifacts/<env>/state/{infra,config}.tfstate` — a mode-`0700` directory holding the non-regenerable, secret-bearing files, with its own backup policy — and disposable saved plans live separately at `../artifacts/<env>/plans/`. Templates are full overlay *directories*, `config/templates/<provider>/<role>/<name>/`, whose `template.yaml` is identity only: topology sits in the template's `placement.yaml` and tuning ships as `values/` and `patches/` overlays, with no `app`/`data`/`tooling` variable sections. The `infra.talos.{nameservers, ntp_servers}` keys also moved out of `config.yaml` into a `talos.yaml` sidecar per environment. Everything else stands.
