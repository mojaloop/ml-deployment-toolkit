...
> **Amended 2026-08-14 (config-layering refactor).** Three mechanics below have moved on. Environments and generated output now live *outside* the clone, as siblings of it: `../environments/<env>/` (each environment its own private git repository) and `../artifacts/<env>/`, with state at `../artifacts/<env>/terraform/{infra,config}.tfstate`. Placement moved out of `config.yaml` into a `placement.yaml` sidecar per environment. And the two-layer template split — generic capacity template plus `config/templates/mappings/<provider>.yaml` — is replaced by provider-specific full overlays at `config/templates/<provider>/<role>/<name>.yaml`, with the mapping folded into `providers/<provider>/params.yaml`. The two stacks, the capability sections, the data modes, the generated internal secrets, and schema validation all stand.

> **Amended 2026-08-15 (config-layering refactor, second wave).** Two paths in the previous amendment have moved again. State now lives at `../artifacts/<env>/state/{infra,config}.tfstate` — a mode-`0700` directory holding the non-regenerable, secret-bearing files, with its own backup policy — and disposable saved plans live separately at `../artifacts/<env>/plans/`. Templates are full overlay *directories*, `providers/<provider>/templates/<role>/<name>/`, whose `template.yaml` is identity only: topology sits in the template's `placement.yaml` and tuning ships as `values/` and `patches/` overlays, with no `app`/`data`/`tooling` variable sections. The `infra.talos.{nameservers, ntp_servers}` keys also moved out of `config.yaml` into a `talos.yaml` sidecar per environment. Everything else stands.
