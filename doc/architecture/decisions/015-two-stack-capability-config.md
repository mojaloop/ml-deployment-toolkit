...
- **Values overrides work for every chart**, because each HelmRelease carries an optional `valuesFrom` its own override ConfigMap. Substitution variables are still not expanded in override files.
