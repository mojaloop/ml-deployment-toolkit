...
> **Amended 2026-08-15 (config-layering refactor, second wave).** The chain gained a middle slot: every HelmRelease now ends with a three-layer tail — a `<targetNamespace>-<release>-values-template` ConfigMap carrying the selected template's tuning, then the override twins — making the precedence chart defaults → distribution → template → adopter, later still winning. The tail is generated mechanically by `tools/generate-valuesfrom.sh`; `check-valuesfrom` only verifies. The distribution's own defaults moved beside their layer at `gitops/<layer>/values/<namespace>/<release>.yaml`, so the `values/<namespace>/<release>.yaml` path suffix is identical across the common, template, and environment layers and an override diffs directly against its base. The no-inline rule is unchanged.
