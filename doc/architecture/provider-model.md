...
Placement targets are provider-typed but the files are not: the environment's `placement.yaml` maps `pg-N` to a physical host on on-prem providers (`pg-1: pve1`) and to an availability zone on AWS (`pg-1: eu-west-1a`). On-prem, each VM is placed individually by the wrapping rule (node *i* of a pool takes `placement[i % len]`); on AWS, where EKS offers no per-instance placement, the same rule is honored by splitting each pool into one single-AZ managed node group per distinct placement entry (`db-pg-1`, `db-pg-2`, …) — an internal materialization detail, invisible to template and environment files. An AWS environment without a placement map keeps unpinned pools the autoscaling group spreads best-effort.
