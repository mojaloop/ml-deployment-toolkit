...
Tokens are **bare upper-snake names only** — no operators, no inline defaults (`${X:-y}` is rejected), and a literal `${` is written `$${...}`. Substitution is strict: an undefined variable fails the reconcile instead of passing through (the Flux version floor for this behaviour is pinned in `tools-versions.yaml`).
...
The same contract extends to Helm values: every HelmRelease in the artifact ends its `valuesFrom` chain with the environment's override twins — a ConfigMap then a Secret, both named `<targetNamespace>-<release>-values-override` and both `optional: true` — so an adopter's values file can override any chart without the artifact changing. Chain completeness is CI-checked (`check-valuesfrom`). See [System overview → Configuration layers](system-overview.md#configuration-layers) for the merge order.
